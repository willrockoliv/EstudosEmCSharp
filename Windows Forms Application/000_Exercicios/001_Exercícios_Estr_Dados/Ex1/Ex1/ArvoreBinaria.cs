...
namespace Ex1
{
    ...
    {
        public Nodo raiz = null; // raiz da árvore
        private int qtdeNodosInternos = 0; // qtde de nos internos
        private int qtdeNodosExternos = 0; // qtde de nos internos
        ...
        /// </summary>
        /// <returns></returns>
        public int QtdeNodosExternos() // devolve a qtde de nós externos
        {
            if (QtdeNodosInternos() == 0)
                return qtdeNodosExternos = 0;
            else
                return qtdeNodosExternos = QtdeNodosInternos() + 1;
        }
        ...
        /// <param name="no"></param>
        /// <returns></returns>
        public Nodo PesquisaValor(int valor, Nodo no)
        {
            if (no.EhExterno())
                return no; // não achou!
        ...
        public int Altura(Nodo no)
        {
            if (no.EhExterno())
            {
                Nodo no_aux = no;
                int altura = 0;
                while (no_aux.GetNoPai() != null)
                {
                    altura++;
                    no_aux = no_aux.GetNoPai();
                }

                if (altura > maiorProfundidadeEncontrada)
                    maiorProfundidadeEncontrada = altura;
            }
